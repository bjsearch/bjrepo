...
def generate_batch():
    ...
    for f in valid_files:
        try:
            ...
            try:
                if is_pdf:
                    ...
                    try:
                        ...
                        error_files.append((f.filename, "Excel 지원이 설치되지 않았습니다"))
                        continue
                    except ReportParseError as e:
                        error_files.append((f.filename, f"파일 오류: {str(e)}"))
                        continue
                ...
                error_files.append((f.filename, f"오류: {str(e)}"))
                continue
            finally:
                if compressed_path and os.path.exists(compressed_path) and (not tmp_path or compressed_path != tmp_path):
                    try:
            ...
            try:
                ...
                error_files.append((f.filename, f"저장 실패: {str(e)}"))
    ...
    if generated_count > 0:
        ...
        return render_template("upload.html.j2", **_get_upload_context(error=error_msg)), 400
