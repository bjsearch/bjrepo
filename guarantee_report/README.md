...
**⚠️ 공유 링크는 로그인 없이 열람할 수 있는 링크지만, 아무나 바로 열리지는 않습니다.**
공유 링크(`/s/<토큰>`)를 열면 먼저 "담당자 휴대폰번호 확인" 화면이 뜨고, 그 리포트를
생성한 담당자(설계사)의 휴대폰번호를 정확히 입력해야만 리포트 내용이 보입니다(한 번
확인하면 같은 브라우저에서는 다시 묻지 않습니다). 리포트를 만든 사람의 계정 정보가
없는(아주 오래된) 리포트는 이 확인 절차 없이 바로 열립니다. 리포트 화면에서 언제든 공유를
...
### 리포트 챗봇 (리포트 내용 기반 Q&A)

리포트 화면 좌측 하단에 챗봇 버튼이 뜨면(설정 시에만 표시), 그 리포트의 계약 현황·보장
진단·추천·핵심 제언 데이터와 `guidelines/insurance_guideline.md`(보험 가이드라인)를 근거로
자유 질문에 답합니다. 예: "실비 보장이 충분한가요?", "가장 시급하게 보완해야 할 건 뭐예요?".
로그인한 화면(`/reports/<id>`)과 공유 링크(`/s/<token>`, 담당자 휴대폰번호 확인 후) 양쪽에서
모두 사용할 수 있습니다.

리포트에 없는 개인정보는 답하지 않고, 특정 상품 가입을 노골적으로 권유하거나 기존 계약
해지(승환)를 유도하지 않도록 시스템 프롬프트에 제약을 걸어두었습니다. 답변은 참고용이며
보험금 지급 근거가 될 수 없습니다.

사용하려면 `ANTHROPIC_API_KEY` 환경변수에 [Anthropic Console](https://console.anthropic.com)에서
발급한 API 키를 설정하세요. **설정하지 않으면 챗봇 버튼 자체가 표시되지 않고**, 나머지 기능은
그대로 동작합니다. 모델은 기본값(`claude-sonnet-5`)을 쓰며, `ANTHROPIC_MODEL` 환경변수로
다른 모델로 바꿀 수 있습니다. API 호출은 유료이므로(질문마다 과금), 분당 요청 수를 리포트/
공유 링크별로 제한해두었습니다(기본 분당 12회).

### Render 배포

저장소 루트의 `render.yaml`로 배포합니다. render.com에서 "New → Blueprint"로 이
저장소를 연결하면, 빌드/실행 명령이 자동으로 설정되고 `APP_PASSWORD`, `DATABASE_URL`,
`ADMIN_PHONES`, `KAKAO_JS_KEY`, `ANTHROPIC_API_KEY` 값을 입력하는 칸이 나타납니다
(레포에는 저장되지 않는 비밀값). `SECRET_KEY`는 Render가 자동으로 랜덤 값을 만들어줍니다.
나머지는 모두 비워두고 나중에 Environment 탭에서 추가/수정해도 됩니다 — `DATABASE_URL`을
비워두면 SQLite로, `ANTHROPIC_API_KEY`를 비워두면 챗봇 버튼이 숨겨진 채로 정상 동작합니다.
