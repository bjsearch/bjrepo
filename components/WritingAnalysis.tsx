  ...
  if (!analysis) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center">
        ...
        <h3 className="text-slate-700 font-semibold text-lg mb-2">AI Writing Coach</h3>
        <p className="text-slate-400 text-sm leading-relaxed">
          Write your diary entry and click <span className="font-medium text-indigo-500">"Analyze My Writing"</span> to get:
        </p>
        <div className="mt-4 grid grid-cols-2 gap-3 text-left">
          {[
            { icon: '✏️', label: 'Grammar corrections' },
            { icon: '💬', label: 'Better sentences' },
            { icon: '🌟', label: 'Modern expressions' },
            { icon: '📊', label: 'Writing score' },
          ].map((item) => (
            <div key={item.label} className="flex items-center gap-2 bg-slate-50 rounded-xl p-3">
              <span>{item.icon}</span>
