...
분석 기록은 별도 서버/DB 없이 브라우저의 `localStorage`에 저장됩니다 (`lib/history.ts`).
같은 브라우저에서 캘린더 탭에 들어가면 날짜별로 점수·분석 내용을 다시 열어볼 수 있고,
필요 없는 기록은 삭제할 수 있습니다. (브라우저/기기를 바꾸면 기록이 보이지 않습니다.)
